...
from model import (
    DODATKOWE_MINUTY, KATEGORIE_SPRAW, PODSTAWOWE_CZYNNOSCI,
    domyslne_parametry, oblicz_model, oblicz_prog_czasu,
    oblicz_prog_kosztu_stalego, oblicz_prog_wynagrodzenia_pracownika, oblicz_prog_fin,
)
...
def tabela_podsumowania(podsumowanie: dict) -> pd.DataFrame:
    return pd.DataFrame([
        ...
        ("Przychód", kwota(podsumowanie["przychod"])), ("Koszt", kwota(podsumowanie["koszt"])),
        ("Wynik", kwota(podsumowanie["wynik"])), ("Średni przychód / sprawa", kwota(podsumowanie["sredni_przychod"])),
        ("Średni koszt / sprawa", kwota(podsumowanie["sredni_koszt"])), ("Średni wynik / sprawa", kwota(podsumowanie["sredni_wynik"])),
...
with st.sidebar:
    ...
    with st.expander("Portfel", expanded=True):
        ...
        fin_percent = st.number_input("Średnia kwota wyroku / ugody (% WPS)", min_value=0.0, max_value=100.0, value=domyslne["fin_percent"], step=0.1, format="%.1f", help="Np. 70% oznacza, że średnia kwota wyroku/ugody wynosi 70% WPS, czyli jest o 30% niższa od pierwotnego WPS.")
    ...
    with st.expander("Czas pracy"):
        ...
        st.write(f"Podstawowy czas: **{minuty_podstawowe} min / {minuty_podstawowe / 60:.2f} h**")
        st.write(f"Podstawowy koszt obsługi sprawy: **{kwota((koszt_staly + wynagrodzenie_pracownika) * minuty_podstawowe / 60)}**")
    with st.expander("Specyfika spraw"):
        ...
        st.caption(f"Udział spraw z WPS poniżej progu: {procent(100.0 - wysoki_wps_procent)}")
...
parametry = {"liczba_spraw": liczba_spraw, "prog_wps": prog_wps, "fin_percent": fin_percent, "niski_wps": niski_wps, "wysoki_wps": wysoki_wps, "koszt_staly_na_godzine": koszt_staly, "wynagrodzenie_pracownika_na_godzine": wynagrodzenie_pracownika, "podstawowe_czynnosci": czynnosci, "dodatkowe_minuty": dodatkowe, "udzialy_rodzajow": udzialy, "wysoki_wps_procent": wysoki_wps_procent}
wyniki = oblicz_model(parametry)
ogolem = wyniki["ogolem"]

kpi = st.columns(4)
kpi[0].metric("Przychód", kwota(ogolem["przychod"])); kpi[1].metric("Koszt", kwota(ogolem["koszt"])); kpi[2].metric("Zysk / strata", kwota(ogolem["wynik"])); kpi[3].metric("Marża", procent(ogolem["marza"]))

st.header("Próg rentowności")
st.caption("Wartości lepsze od wskazanego progu prowadzą do zysku, przy pozostałych założeniach bez zmian.")
czas, koszt, efektywnosc = st.columns(3)
with czas:
    st.subheader("Czas")
...
with niski:
    st.subheader(f"WPS poniżej progu ({prog_wps:,.0f} zł)".replace(",", " ")); st.dataframe(tabela_podsumowania(wyniki["wps_niski"]), hide_index=True, width="stretch")
with wysoki:
    st.subheader(f"WPS od progu wzwyż ({prog_wps:,.0f} zł)".replace(",", " ")); st.dataframe(tabela_podsumowania(wyniki["wps_wysoki"]), hide_index=True, width="stretch")
...
podzial_tabela = pd.DataFrame([{"Rodzaj": rodzaj, "Liczba spraw": sum(wartosci.values()), "WPS od progu wzwyż": wartosci["wysoki_wps"], "WPS poniżej progu": wartosci["niski_wps"]} for rodzaj, wartosci in wyniki["podzial_spraw"].items()])
...
rodzaje = pd.DataFrame([{"Rodzaj": rodzaj, "Opis": KATEGORIE_SPRAW[rodzaj], "Liczba spraw": dane["liczba_spraw"], "Udział w portfelu": dane["liczba_spraw"] / ogolem["liczba_spraw"] * 100 if ogolem["liczba_spraw"] else 0.0, "Czas jednej sprawy (h)": (wyniki["podstawowe_minuty"] + dodatkowe[rodzaj]) / 60, "Koszt jednej sprawy": dane["sredni_koszt"], "Łączny przychód": dane["przychod"], "Łączny koszt": dane["koszt"], "Łączny wynik": dane["wynik"], "Średni wynik na sprawie": dane["sredni_wynik"]} for rodzaj, dane in wyniki["rodzaje"].items()])
st.dataframe(rodzaje, hide_index=True, width="stretch", column_config={"Udział w portfelu": st.column_config.NumberColumn(format="%.1f%%"), "Czas jednej sprawy (h)": st.column_config.NumberColumn(format="%.2f"), "Koszt jednej sprawy": st.column_config.NumberColumn(format="%.2f zł"), "Łączny przychód": st.column_config.NumberColumn(format="%.2f zł"), "Łączny koszt": st.column_config.NumberColumn(format="%.2f zł"), "Łączny wynik": st.column_config.NumberColumn(format="%.2f zł"), "Średni wynik na sprawie": st.column_config.NumberColumn(format="%.2f zł")})

kolumna_1, kolumna_2 = st.columns(2)
with kolumna_1:
    st.subheader("Przychód vs koszt vs wynik"); st.bar_chart(pd.DataFrame({"Kwota": [ogolem["przychod"], ogolem["koszt"], ogolem["wynik"]]}, index=["Przychód", "Koszt", "Wynik"]))
with kolumna_2:
    st.subheader("Wynik według rodzaju sprawy"); st.bar_chart(rodzaje.set_index("Rodzaj")[["Łączny wynik"]])
